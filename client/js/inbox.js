...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const btnFecharAnexoViewer = document.getElementById(
    "btn-fechar-anexo-viewer"
  );
  ...
  const checkboxSalvarMapeamento = document.getElementById(
    "checkbox-salvar-mapeamento"
  );
  ...
  const btnSubmitImportacao = document.querySelector(
    '#form-importacao-extrato button[type="submit"]'
  );
  ...
  async function carregarInbox(params = {}) {
    ...
    try {
      ...
      transacoes.forEach((t) => {
        ...
        const statusClasse =
          t.status === "PENDENTE" ? "status-pendente" : "status-processado";
        ...
        tr.innerHTML = `
                    <td><input type="checkbox" class="checkbox-item" data-id="${
                      t.id
                    }"></td>
                    <td>${formatarData(t.data)}</td>
                    <td>${t.descricao || "—"}</td>
                    <td class="valor-cell ${
                      t.tipo === "RECEITA" ? "positivo" : "negativo"
                    }">
    ...
    try {
      ...
      if (anexos.length > 0) {
        ...
        const placeholder = document.querySelector(
          `.receipt-icon-placeholder[data-transacao-id="${transacaoId}"]`
        );
        if (placeholder) {
  ...
  formFiltros.addEventListener("submit", (e) => {
    ...
    const statusRadio = document.querySelector(
      'input[name="filtro_status"]:checked'
    );
    const params = {
      ...
      sem_categoria:
        document.getElementById("filtro-sem-categoria").checked || undefined,
      sem_perfil:
        document.getElementById("filtro-sem-perfil").checked || undefined,
    };
  ...
  formAplicarMassa.addEventListener("submit", async (e) => {
    ...
    if (
      !confirm(
        `Você está prestes a processar ${ids.length} transações. Esta ação não pode ser desfeita. Deseja continuar?`
      )
    ) {
      return;
    }
    ...
    try {
      const resposta = await api.categorizarEmMassa(
        ids,
        id_categoria,
        id_perfil
      );
  ...
  async function handleAbrirModalEdicao(id) {
    ...
    try {
      ...
      if (anexos && anexos.length > 0 && containerAnexo) {
        ...
        btnVerAnexo.onclick = () =>
          handleAbrirAnexoViewer(anexo.caminho_storage);
      }
    } catch (error) {
  ...
  formEditTransacao.addEventListener("submit", async (e) => {
    ...
    if (vaiProcessar) {
      if (
        !confirm(
          "Você está prestes a processar esta transação. Esta ação não pode ser desfeita. Deseja continuar?"
        )
      ) {
        return;
      }
  ...
  document
    .getElementById("btn-cancelar-massa")
    .addEventListener("click", fecharModais);
  modalFiltros.addEventListener("click", (e) => {
    if (e.target === modalFiltros) fecharModais();
  });
  modalMassa.addEventListener("click", (e) => {
    if (e.target === modalMassa) fecharModais();
  });
  modalEdit.addEventListener("click", (e) => {
    if (e.target === modalEdit) fecharModais();
  });
  modalAnexoViewer.addEventListener("click", (e) => {
    if (e.target === modalAnexoViewer) fecharModais();
  });
  btnCancelarImport.addEventListener("click", fecharModais);
  modalImportacao.addEventListener("click", (e) => {
    if (e.target === modalImportacao) fecharModais();
  });
  ...
  checkboxSalvarMapeamento.addEventListener(
    "change",
    handleSalvarMapeamentoToggle
  );
  ...
  async function carregarMapeamentosSalvos() {
    try {
      ...
      if (mappingSelecionadoAtual) {
        const stillExists = mapeamentos.find(
          (m) => m.id === mappingSelecionadoAtual.id
        );
      ...
      selectMapeamento.innerHTML =
        '<option value="">Nenhum mapeamento disponível</option>';
    }
    atualizarEstadoSalvarMapeamento();
  }

  function handleSelecaoMapeamentoChange() {
    mappingSelecionadoAtual =
      mapeamentosSalvosCache.find((m) => m.id === selectMapeamento.value) ||
      null;
  ...
  function prepararPreviewCSV(texto) {
    ...
    colunasCSVDetectadas = Array.from({ length: totalColunas }, (_, index) => ({
      ...
      samples: linhasSeparadas
        .map((linha) => linha[index] || "")
        .filter(Boolean)
        .slice(0, 6),
      mappedType: "",
      selectEl: null,
  ...
  async function handleImportacaoExtrato(event) {
    ...
    try {
      ...
      alert(
        `Importação concluída! ${resultado.total_importadas} transações foram enviadas para a Inbox.`
      );
      fecharModais();
      carregarInbox();
      notificarAtualizacaoDashboard();
