...
try:
    ...
    from infra.db.database import Session, init_db
...
def seed_initial_data():
    ...
    try:
        ...
        perfis_seed = {
            "perfil_pessoal": Perfil(
                id="perfil_pessoal", id_usuario=user_id, nome="Pessoal"
            ),
        ...
        categorias_seed = {
            ...
            "cat_lazer": Categoria(
                id="cat_lazer",
                id_usuario=user_id,
                nome="Lazer (Cinema, Bares)",
                tipo=TipoTransacao.DESPESA,
            ),
            "cat_educacao": Categoria(
                id="cat_educacao",
                id_usuario=user_id,
                nome="Educação (Cursos, Livros)",
                tipo=TipoTransacao.DESPESA,
            ),
            "cat_vestuario": Categoria(
                id="cat_vestuario",
                id_usuario=user_id,
                nome="Vestuário",
                tipo=TipoTransacao.DESPESA,
            ),
            "cat_servicos_app": Categoria(
                id="cat_servicos_app",
                id_usuario=user_id,
                nome="Serviços (Streaming, Apps)",
                tipo=TipoTransacao.DESPESA,
            ),
            "cat_impostos": Categoria(
                id="cat_impostos",
                id_usuario=user_id,
                nome="Impostos e Taxas",
                tipo=TipoTransacao.DESPESA,
            ),
            "cat_invest": Categoria(
                id="cat_invest",
                id_usuario=user_id,
                nome="Investimentos (Aportes)",
                tipo=TipoTransacao.DESPESA,
            ),
            "cat_viagem": Categoria(
                id="cat_viagem",
                id_usuario=user_id,
                nome="Viagem",
                tipo=TipoTransacao.DESPESA,
            ),
            "cat_outros_d": Categoria(
                id="cat_outros_d",
                id_usuario=user_id,
            ...
            "cat_renda_sup": Categoria(
                id="cat_renda_sup",
                id_usuario=user_id,
                nome="Renda Suplementar (Freela)",
                tipo=TipoTransacao.RECEITA,
            ),
            "cat_outros_r": Categoria(
                id="cat_outros_r",
                id_usuario=user_id,
