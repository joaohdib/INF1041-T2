...
from domain.transacao import StatusTransacao, Transacao
...
def lancar_transacao_route():
    ...
    try:
        ...
        payload = {
            ...
            "data": datetime.fromisoformat(data_form.get("data"))
            if data_form.get("data")
            else datetime.now(),
        ...
        if "anexo" in request.files:
            ...
            if file and file.filename != "":
                ...
                use_case_anexar = AnexarReciboTransacao(
                    storage, anexo_repo, transacao_repo
                )
...
def deletar_transacao_route(id_transacao: str):
    ...
    try:
        ...
        use_case.execute(id_usuario=id_usuario, id_transacao=id_transacao)
...
def importar_extrato_route():
    ...
    try:
        ...
        if salvar_nome:
            if not mapping:
                return jsonify(
                    {"erro": "Para salvar o mapeamento informe as colunas."}
                ), 400
...
def filtrar_inbox_route():
    ...
    try:
        ...
        # Converte tipos
        data_de = (
            date.fromisoformat(args.get("data_de")) if args.get("data_de") else None
        )
        data_ate = (
            date.fromisoformat(args.get("data_ate")) if args.get("data_ate") else None
        )
        valor_min = (
            float(args.get("valor_min")) if args.get("valor_min") is not None else None
        )
        valor_max = (
            float(args.get("valor_max")) if args.get("valor_max") is not None else None
        )
