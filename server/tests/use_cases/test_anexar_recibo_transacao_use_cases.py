...
def test_anexar_recibo_sucesso(
    use_case, mock_storage, mock_anexo_repo, mock_transacao_repo
):
    ...
    mock_file = Mock(
        content_length=1024, filename="recibo.pdf", mimetype="application/pdf"
    )
    ...
    mock_storage.save.assert_called_once_with(
        mock_file, "recibo.pdf", "application/pdf"
    )
...
def test_anexar_recibo_falha_formato_nao_suportado(
    use_case, mock_storage, mock_anexo_repo
):
...
def test_anexar_recibo_falha_transacao_nao_encontrada(
    use_case, mock_transacao_repo, mock_storage
):
    # 1. Arrange
...
def test_anexar_recibo_falha_seguranca_outro_usuario(
    use_case, mock_transacao_repo, mock_storage
):
    # 1. Arrange
    id_usuario = "user_1"
