...
class AnexarReciboTransacao:
    ...
    def execute(
        ...
        if content_type not in self.ALLOWED_MIMES:
            raise ValueError(
                f"Formato de arquivo não suportado: {content_type}. (Permitidos: JPG, PNG, PDF)"
            )
...
class AtualizarTransacao:
    ...
    def execute(
        self, id_usuario: str, id_transacao: str, dados_atualizados: Dict[str, Any]
    ) -> Transacao:
        transacao = self.transacao_repo.get_by_id(id_transacao)
        if not transacao:
            raise ValueError("Transação não encontrada.")
    ...
    def execute(
        ...
        for t in transacoes:
            if t.id_usuario != id_usuario:
                print(
                    f"Aviso: Tentativa de categorizar transação {t.id} de outro usuário."
                )
                continue
